...
// Make sure this imports the class we just defined in the service
...
class PatternRecognitionManager(private val context: Context) {
    ...
    /**
     * Finds menu buttons by reading their text
     */
    private fun findSmartAction(nodes: List<UIContextNode>): ActionSequence? {
        // Keywords for Domino Dreams and generic Android dialogs
    ...
    /**
     * Finds matching game pieces by comparing grid cells
     */
    private fun findVisualMatches(bitmap: Bitmap): ActionSequence? {
        val width = bitmap.width
        val height = bitmap.height
        // Focus on the center board area
        ...
        try {
            ...
            for (i in 0 until cells.size) {
                for (j in i + 1 until cells.size) {
                    ...
                    // 92% similarity threshold for a match
                    if (similarity > 0.92f) { 
                        return ActionSequence(
                            actionType = "CLICK",
                            x = cellA.centerX,
    ...
    data class PatternTemplate(
        val type: String, val features: FloatArray, val x: Float, val y: Float,
        val width: Int, val height: Int, var observationCount: Int = 0, var lastSeen: Long = 0L
    )

    private data class GridCell(
        val row: Int, val col: Int, val centerX: Float, val centerY: Float, val features: FloatArray
    )
}

class ImagePreprocessor {
    fun preprocessImage(bitmap: Bitmap): TensorImage = TensorImage.fromBitmap(bitmap)
}
